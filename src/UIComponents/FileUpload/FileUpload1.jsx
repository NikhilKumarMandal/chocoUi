...
const FileUpload1 = () => {
  ...
  const updateSpecificProgress = useCallback(
    (fileName, progress) => {
    ...
    (fileName) => {
      ...
      setUploadProgress((prev) => {
        ...
        if (
          ...
          remainingFiles.every((f) => newProgress[f.name] === 100)
        ) {
          setIsUploading(false);
        }
        return newProgress;
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center space-y-3">
          ...
          <p className="text-xs text-gray-500">
            {ALLOWED_TYPES.map((t) => t.split("/")[1].toUpperCase()).join(", ")}{" "}
            up to {formatFileSize(MAX_SIZE)}
          </p>
        </div>
      </div>
                ...
                <li key={file.name} className="px-4 py-3">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="flex items-center flex-grow min-w-0 mr-4">
                      <svg
                        className="h-6 w-6 text-gray-400 flex-shrink-0 mr-3"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
