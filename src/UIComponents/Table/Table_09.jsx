...
const Table_09 = () => {
      ...
      setTasks(tasks.map(task => {
        if (task.id === taskId) {
          ...
          return {
            ...task,
            isCompleted,
            status: isCompleted ? "Completed" : task.status === "Completed" ? "In Progress" : task.status
          };
        }
        return task;
      }));
    ...
    const handleAddTask = () => {
      const newTask = {
        ...
        assignee: "Unassigned",
        initials: "UN",
        dueDate: new Date().toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }),
        status: "Pending",
        priority: "Medium",
        isCompleted: false
      };
    ...
    const renderMobileCard = (task) => (
      <div key={task.id} className="bg-white p-4 rounded-lg shadow mb-4 border border-gray-100">
