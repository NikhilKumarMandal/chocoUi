...
function Carousel1() {
  ...
  const containerClass =
    "relative w-full max-w-3xl mx-auto overflow-hidden rounded-2xl shadow-lg";
  const imageClass = "w-full object-cover transition duration-500";
  const buttonBase =
    "absolute top-1/2 -translate-y-1/2 bg-white bg-opacity-60 hover:bg-opacity-80 text-black p-2 rounded-full transition-all duration-300 shadow-md";
  ...
  return (
    <div className={containerClass}>
      <img
        ...
        alt={"Slide " + (current + 1)}
      />
      <button onClick={prev} className={[buttonBase, buttonLeft].join(" ")}>
        <ArrowLeft size={24} />
      </button>
      <button onClick={next} className={[buttonBase, buttonRight].join(" ")}>
        <ArrowRight size={24} />
      </button>
    </div>
  );
