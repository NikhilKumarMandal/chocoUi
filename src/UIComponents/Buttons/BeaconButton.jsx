...
const BeaconButton = ({
  ...
  const wrapperClass = center ? "flex items-center justify-center " : "";

  const buttonClasses =
    "group relative flex h-12 w-44 m-4 items-center justify-between overflow-hidden " +
    "rounded-full px-6 py-3 font-medium bg-[#7A4D28] text-[#FCD9B8] " +
    "transition-all duration-300 hover:bg-[#5C371B] hover:shadow-lg " +
    "dark:bg-[#FFF8F2] dark:text-[#3B2A1A] dark:hover:bg-[#F5E8DA] " +
    "before:absolute before:inset-0 before:rounded-full before:border-2 before:border-transparent " +
    "before:transition-all before:duration-500 hover:before:border-[#FCD9B8]/30 hover:dark:before:border-[#3B2A1A]/30 " +
  ...
  const dotClasses =
    "absolute right-5 h-2 w-2 rounded-full bg-white/60 opacity-0 " +
    "transition-all duration-500 group-hover:opacity-100 " +
    "group-hover:[animation:pulse_1.5s_infinite] dark:bg-[#3B2A1A]/70";
