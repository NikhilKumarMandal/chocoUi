      ...
      codeSnippets: [
        {
          ...
          const Loader = () => {
            return (
              <div className="h-12 w-12 border-4 border-cyan-400 animate-[spin_1s_linear_infinite]"></div>
            );
          };
...
const Loader = () => {
  return (
    <div className="flex space-x-1 items-end h-10">
      {[...Array(5)].map((_, i) => {
        const bounceClass = "w-2 bg-cyan-500 animate-[bounce_1s_ease-in-out_infinite]";
        const delayStyle = { animationDelay: (i * 0.2).toString() + "s", height: "100%" };
...
const Loader = ({ size = "md", color = "blue" }) => {
  ...
  return (
    <div className="flex justify-center items-center">
      <div
        className={
          sizeClasses[size] + " border-4 border-t-transparent rounded-full animate-spin " + colorClasses[color]
        }
      ></div>
    </div>
  );
...
const Loader_15 = ({ size = "md", color = "blue" }) => {
  const sizeClasses = {
    sm: "w-6 h-6",
    md: "w-10 h-10",
    lg: "w-16 h-16",
